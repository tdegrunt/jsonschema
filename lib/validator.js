...
Validator.prototype.validate = function validate(instance, schema, options) {
  if (schema) {
...
Validator.prototype.validateSchema = function validateSchema(instance, schema, options) {
...
Validator.prototype.validateProperty = function validateProperty(instance, schema, options) {
  var self=this;
  var valid;
  if (!options) {
  ...
  if(schema.$ref) {
    if (this.schemas[schema.$ref]) {
      this.validateSchema(instance, this.schemas[schema.$ref], options);
    } else {
      self.addError(helpers.createError(schema, '$ref', options.propertyName, "no such schema " + schema.$ref));
    }
    return;
  }
  var keys = Object.keys(schema);
  keys.forEach(function(key){
    if (options.skipAttributes.indexOf(key)<0 && attribute.ignoreProperties.indexOf(key)<0) {
      var validatorErr;
      ...
      if (options.addError && validatorErr) {
        self.addError(helpers.createError(schema, key, options.propertyName, validatorErr));
      }
    }
  });
  return valid;
