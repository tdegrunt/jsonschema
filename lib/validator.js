...
Validator.prototype.validateSchema = function validateSchema(instance, schema, options, propertyName) {
  ...
  if(switchSchema) {
    if (this.schemas[switchSchema]) {
      this.validateSchema(instance, this.schemas[switchSchema], options, propertyName);
    } else {
      this.addError(helpers.createError(schema, '$ref', propertyName, "no such schema " + switchSchema));
    }
    return;
  }
  this.validateProperty(instance, schema, options, propertyName);
  if(instance instanceof Array){
    this.validateArray(instance, schema, options, propertyName);
  }else if(instance && (typeof instance)=='object' && Object.getPrototypeOf(instance)===Object.prototype){
    this.validateObject(instance, schema, options, propertyName);
  }
};
