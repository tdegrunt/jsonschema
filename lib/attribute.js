...
validators.anyOf = function validateAnyOf (instance, schema, options, ctx) {
  ...
  if (!schema.anyOf.some(testSchema.bind(this, instance, options, ctx))) {
    return "is not any of " + schema.anyOf.map(function (v) {
      return (v.id && ('<' + v.id + '>')) || (v.title && JSON.stringify(v.title)) || (v['$ref'] && ('<' + v['$ref'] + '>')) || '<subschema>';
    });
  }
  return null;
};
...
validators.required = function validateRequired (instance, schema, options, ctx) {
  if (schema.required === true && (!instance || instance == '')) {
    return "is required";
  }else if (instance && typeof instance==='object' && Array.isArray(schema.required)) {
    var result = new ValidatorResult(instance, schema, options, ctx);
    schema.required.forEach(function(n){
      if(!instance[n] || instance[n] == ''){
        result.addError("is required", n);
      }
    });
    return result;
  }
...
validators.pattern = function validatePattern (instance, schema) {
  ...
  if (!(instance || '').toString().match(schema.pattern)) {
...
validators.format = function validateFormat (instance, schema, options, ctx) {
  ...
  if (!helpers.isFormat((instance || '').toString(), schema.format)) {
