...
validators.properties = function validateProperties (instance, schema, options, ctx) {
  ...
  for (var property in properties) {
    ...
    if(res.errors.length==0 && instance[property]) {
        instance[property] = replaceFormatBySpecificObject(this, properties[property], instance[property]);
    }
  }
  return result;
};
...
/**
 * If the current validator contains a structure named "replacingValueIfStringIsFormat",
 * It checks if the property "format" attribute has an associated type defined in "replacingValueIfStringIsFormat".
 * In that case, the instance property string value is replaced by the configured replacing type object.
 * @param validator the validator instance on which we check the existence of a structure named "replacingValueIfStringIsFormat" in which formats are associated to a type
 * @param element the element in schema ("property" or "items")
 * @param value current instance value in JSON
 * @private
 */
function replaceFormatBySpecificObject (validator, element, value) {
    if(!validator.replacingValueIfStringIsFormat) {
        return value;
    } else {
        var ReplacingType = validator.replacingValueIfStringIsFormat[element.format];
        if(ReplacingType) {
            return new ReplacingType(value);
        } else {
            return value;
        }
    }
...
function testAdditionalProperty (instance, schema, options, ctx, property, result) {
  ...
  if (schema.additionalProperties === false) {
      if(this.removePropertiesIfNotInSchema) {
        delete result.instance[property];
      }
      else {
...
validators.items = function validateItems (instance, schema, options, ctx) {
  ...
  instance.every(function (value, i) {
    ...
    if(res.errors.length==0 && res.instance) {
      result.instance[i] = replaceFormatBySpecificObject(self, items, res.instance);
    }
    result.importErrors(res);
    return true;
  });
