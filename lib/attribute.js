...
// Only applicable for arrays, ignored otherwise
Attribute.prototype.validateUniqueField = function () {
  var i;
  var valid = true;
  ...
  if (helpers.isArray(this.instance)) {
    ...
    for (i = 0; i < this.instance.length; i++) {
      var objToCheck = this.instance[i];
      var valueToCheck = objToCheck[this.value];
      if (foundItems.indexOf(valueToCheck) !== -1) {
        // stop as soon as a duplicate value is found
        valid = false;
        break;
      };
      foundItems.push(valueToCheck);
    };
    if(!valid) {
      return this.createError(this.value + " is not unique");
    }
  }
};
